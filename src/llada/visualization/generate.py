...
def main():
    device = 'cuda'
    ds = load_dataset("DigitalLearningGmbH/MATH-lighteval", cache_dir="/weka/geiger/gwb130/dlm-r1/cache")["test"]
    d = ds[3412]
    problem, solution, level, p_type = d["problem"], d["solution"], d["level"], d["type"]
    tokenizer = AutoTokenizer.from_pretrained('GSAI-ML/LLaDA-8B-Instruct', trust_remote_code=True, cache_dir="/weka/geiger/gwb130/dlm-r1/cache")
    model = AutoModel.from_pretrained('GSAI-ML/LLaDA-8B-Instruct', trust_remote_code=True, torch_dtype=torch.bfloat16,
                                      cache_dir="/weka/geiger/gwb130/dlm-r1/cache", device_map=device)

    prompt = problem + "Let's think step by step and output the final answer within \\boxed{}."
    ...
    out = generate(model, input_ids, tokenizer, steps=256, gen_length=512, block_length=8, temperature=0., cfg_scale=0., remasking='low_confidence')
